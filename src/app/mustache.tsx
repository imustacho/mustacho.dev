...
// Original absolute coordinates of the detailed vector mustache
const POINTS = [
    ...
    { type: 'C', x1: 4405, y1: 5317, x2: 4400, y2: 5316, x: 4400, y: 5251 }, // Left Tip
    ...
    { type: 'C', x1: 10890, y1: 4951, x2: 10990, y2: 5152, x: 10990, y: 5243 }, // Right Tip
...
export default function Mustache() {
    ...
    const handlePointerMove = (e: React.PointerEvent<SVGPathElement>) => {
        ...
        leftX.set(dragInfo.current.startValLX + dx * dragInfo.current.weightLeft);
        leftY.set(dragInfo.current.startValLY + dy * dragInfo.current.weightLeft);

        rightX.set(dragInfo.current.startValRX + dx * dragInfo.current.weightRight);
        rightY.set(dragInfo.current.startValRY + dy * dragInfo.current.weightRight);
    };
    ...
    const handlePointerUp = (e: React.PointerEvent<SVGPathElement>) => {
        ...
        animate(leftX, 0, { type: "spring", stiffness: 450, damping: 15 });
        animate(leftY, 0, { type: "spring", stiffness: 450, damping: 15 });
        animate(rightX, 0, { type: "spring", stiffness: 450, damping: 15 });
        animate(rightY, 0, { type: "spring", stiffness: 450, damping: 15 });
    };

    return (
        <motion.svg
            viewBox="420 412 700 200"
            className="mt-8 w-72 overflow-visible md:w-96 select-none"
            initial={{ filter: "drop-shadow(0px 6px 12px rgba(149, 86, 35, 0.12))" }}
            whileHover={{ filter: "drop-shadow(0px 14px 24px rgba(149, 86, 35, 0.25))" }}
        ...
        >
            ...
            >
                {/* Render the dynamically warped detailed mustache */}
